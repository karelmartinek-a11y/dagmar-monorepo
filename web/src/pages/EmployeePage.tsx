...
function DayCard({ day, locked, savedField, onSave, onStatus }: { day:AttendanceDay;locked:boolean;savedField:TimeField|null;onSave:(day:AttendanceDay,values:TimeValues,field:TimeField)=>void;onStatus:(day:AttendanceDay,status:string,confirmed?:boolean)=>void }) {
  ...
  const date=asPragueDate(day.date);const hasWholeDayStatus=day.planned_status==="HOLIDAY"||day.planned_status==="OFF";const disabled=locked||!day.is_within_employment_period||hasWholeDayStatus;
...
function PlanDayCard({ day, locked, editable, onSave }: { day:AttendanceDay;locked:boolean;editable:boolean;onSave:(day:AttendanceDay,values:PlanValues,status:string)=>void }) {
  ...
  const date=asPragueDate(day.date);const statusValue=day.planned_status??"";const disabled=locked||!editable||!day.is_within_employment_period||statusValue==="HOLIDAY"||statusValue==="OFF";
...
export function EmployeePage(){
  ...
  return <div className={`employee-app ${inverted?"employee-app--light":""}`}><header className="topbar employee-topbar"><Brand compact/><span>{session.display_name}</span><div className="employee-topbar__actions"><LanguageSwitcher compact /><Button variant="quiet" onClick={()=>setInverted(value=>!value)} aria-label={t("employee.topbar.invertAria")}><Contrast/><span>{t("employee.topbar.invert")}</span></Button><Button variant="quiet" onClick={logout} aria-label={t("employee.topbar.logoutAria")}><LogOut/><span>{t("employee.topbar.logout")}</span></Button></div></header><main id="main-content" className="employee-main"><div className="page employee-page"><h1 className="sr-only">{t("employee.page.title")}</h1><header className="employee-command" aria-label={t("employee.page.summaryLabel")}><button className="icon-button" aria-label={t("employee.month.previous")} onClick={()=>moveMonth(-1)} title={t("employee.month.previous")}><ChevronLeft/></button><div className="employee-command__month" title={t("employee.month.visibleMonth",{month:monthName.format(month)})}><CalendarDays/><strong>{monthName.format(month)}</strong></div><MetricPill icon={<CalendarClock/>} label={t("employee.metrics.plannedHours")} value={shortHoursLabel(plannedMinutes,t)} title={t("employee.metrics.plannedDetail",{hours:hoursLabel(plannedMinutes,t)})}/><MetricPill icon={<BriefcaseBusiness/>} label={t("employee.metrics.worked")} value={shortHoursLabel(actualMinutes,t)} title={t("employee.metrics.workedDetail",{hours:hoursLabel(actualMinutes,t),days:filledDays})}/><MetricPill icon={<Plane/>} label={t("employee.metrics.holiday")} value={t("employee.units.daysShort",{count:holidayDays})} title={t("employee.metrics.vacationDays",{count:holidayDays})}/><MetricPill className="employee-pill--processing" icon={navigator.onLine?<Wifi/>:<WifiOff/>} label={t("employee.metrics.processing")} value={query.data?.locked?t("employee.metrics.lock"):queueCount?t("employee.metrics.waiting",{count:queueCount}):navigator.onLine?t("employee.metrics.online"):t("employee.metrics.offline")} title={query.data?.locked?t("employee.metrics.monthLocked"):queueCount?t("employee.metrics.queueWaiting",{count:queueCount}):navigator.onLine?t("employee.metrics.onlineDirect"):t("employee.metrics.offlineQueued")}/><button className="icon-button" aria-label={t("employee.month.next")} onClick={()=>moveMonth(1)} title={t("employee.month.next")}><ChevronRight/></button></header><div className="employee-mode-switch" role="tablist" aria-label={t("employee.page.switchView")}><button type="button" role="tab" aria-selected={view==="attendance"} className={view==="attendance"?"active":""} onClick={()=>setView("attendance")}><CalendarDays/>{t("employee.page.attendanceTab")}</button><button type="button" role="tab" aria-selected={view==="plan"} className={view==="plan"?"active":""} onClick={()=>setView("plan")}><CalendarRange/>{t("employee.page.planTab")}</button></div><PanelToolbar session={session} onSession={setSession}/>{notice&&<StatusMessage kind={queueCount?"offline":"success"} title={queueCount?t("employee.notices.queuedTitle"):t("employee.notices.doneTitle")}>{notice}</StatusMessage>}{(mutation.error||statusMutation.error||planMutation.error)&&<StatusMessage kind="error" title={t("employee.errors.saveFailed")}>{(mutation.error||statusMutation.error||planMutation.error)?.message}</StatusMessage>}{query.isPending&&<StatusMessage kind="loading" title={t("employee.errors.loading")}/>} {query.error&&<StatusMessage kind="error" title={t("employee.errors.loadFailed")}>{query.error.message}</StatusMessage>}{query.data&&<>{query.data.locked&&<StatusMessage kind="offline" title={t("employee.errors.lockedTitle")}>{t("employee.errors.lockedBody")}</StatusMessage>}{view==="plan"&&!(query.data.shift_plan_editable ?? false)&&<StatusMessage kind="offline" title={t("employee.errors.planReadonlyTitle")}>{t("employee.errors.planReadonlyBody")}</StatusMessage>}<section className="ledger employee-ledger" aria-label={view==="attendance"?t("employee.page.attendanceDays"):t("employee.page.shiftPlanDays")}>{query.data.days.map(day=>view==="attendance"?<DayCard key={day.date} day={day} locked={query.data.locked} savedField={savedCell?.date===day.date?savedCell.field:null} onSave={(d,values,field)=>mutation.mutate({day:d,values,field})} onStatus={(d,status,confirmed)=>statusMutation.mutate({day:d,status,confirmed})}/>:<PlanDayCard key={day.date} day={day} locked={query.data.locked} editable={query.data.shift_plan_editable ?? false} onSave={(d,values,status)=>planMutation.mutate({day:d,values,status})}/>)}</section></>}{statusConflict&&<Modal title={t("employee.errors.conflictTitle")} description={t("employee.errors.conflictBody",{date:statusConflict.day.date})} confirmLabel={t("employee.errors.conflictConfirm")} danger onClose={()=>setStatusConflict(null)} onConfirm={()=>statusMutation.mutate({...statusConflict,confirmed:true})}/>}</div></main>{view==="attendance"&&<footer className="employee-nowbar" aria-label={t("employee.quickNow.label")}><button type="button" disabled={nowDisabled} onClick={saveNow}><TimerReset/><span>{t("employee.quickNow.button")}</span></button></footer>}</div>;
}
