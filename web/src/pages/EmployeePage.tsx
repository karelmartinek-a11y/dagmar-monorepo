...
import { BriefcaseBusiness, CalendarDays, CalendarRange, ChevronLeft, ChevronRight, Contrast, LogOut, MoreVertical, Plane, TimerReset, Wifi, WifiOff } from "lucide-react";
...
function shortDaysHoursLabel(days:number,minutes:number,t:(key:string, options?: Record<string, unknown>)=>string){return days>0?`${t("employee.units.daysShort",{count:days})} / ${shortHoursLabel(minutes,t)}`:shortHoursLabel(minutes,t);}
...
function DayCard({ day, attendanceLocked, savedField, onSave, onStatus }: { day:AttendanceDay;attendanceLocked:boolean;savedField:TimeField|null;onSave:(day:AttendanceDay,values:TimeValues,field:TimeField)=>void;onStatus:(day:AttendanceDay,status:string,confirmed?:boolean)=>void }) {
  ...
  const statusLabels:Record<string,string>={HOLIDAY:t("employee.statuses.HOLIDAY"),OFF:t("employee.statuses.OFF"),SICKNESS:t("employee.statuses.SICKNESS"),PARAGRAPH:t("employee.statuses.PARAGRAPH")};
  ...
  const date=asPragueDate(day.date);const hasWholeDayStatus=Boolean(day.effective_status);const disabled=attendanceLocked||!day.is_within_employment_period||hasWholeDayStatus;
...
export function EmployeePage(){
  ...
  const actualMinutes=query.data?.summary?.worked_minutes ?? query.data?.days.reduce((total,day)=>total+dayMinutes(day),0)??0;
  const plannedMinutes=query.data?.summary?.planned_minutes ?? query.data?.days.reduce((total,day)=>total+durationMinutes(day.planned_arrival_time,day.planned_departure_time),0)??0;
  const holidayDays=query.data?.summary?.vacation_days ?? query.data?.days.filter(day=>day.planned_status==="HOLIDAY").length??0;
  ...
  return <div className={`employee-app ${inverted?"employee-app--light":""}`}>
    ...
    <main id="main-content" className="employee-main"><div className="page employee-page"><h1 className="sr-only">{t("employee.page.title")}</h1>
      <header className="employee-command" aria-label={t("employee.page.summaryLabel")}>
        ...
        <div className="employee-command__metrics"><MetricPill icon={<BriefcaseBusiness/>} label={t("employee.metrics.workFund")} value={shortHoursLabel(summary?.work_fund_minutes ?? 0,t)} title={t("employee.metrics.workFundDetail",{hours:hoursLabel(summary?.work_fund_minutes ?? 0,t)})}/><MetricPill icon={<CalendarDays/>} label={t("employee.metrics.shiftPlan")} value={shortHoursLabel(plannedMinutes,t)} title={t("employee.metrics.plannedDetail",{hours:hoursLabel(plannedMinutes,t)})}/><MetricPill icon={<BriefcaseBusiness/>} label={t("employee.metrics.worked")} value={shortHoursLabel(actualMinutes,t)} title={t("employee.metrics.workedDetail",{hours:hoursLabel(actualMinutes,t),days:filledDays})}/><MetricPill icon={<Plane/>} label={t("employee.metrics.holiday")} value={shortDaysHoursLabel(holidayDays,summary?.vacation_minutes ?? 0,t)} title={t("employee.metrics.vacationDays",{count:holidayDays})}/><MetricPill icon={<Plane/>} label={t("employee.metrics.sickness")} value={t("employee.units.daysShort",{count:summary?.sickness_days ?? 0})} title={t("employee.metrics.sicknessDetail",{count:summary?.sickness_days ?? 0})}/><MetricPill icon={<Plane/>} label={t("employee.metrics.paragraph")} value={shortHoursLabel(summary?.paragraph_minutes ?? 0,t)} title={t("employee.metrics.paragraphDetail",{hours:hoursLabel(summary?.paragraph_minutes ?? 0,t)})}/><MetricPill icon={<Plane/>} label={t("employee.metrics.afternoon")} value={shortHoursLabel(summary?.afternoon_minutes ?? 0,t)} title={t("employee.metrics.afternoonDetail",{hours:hoursLabel(summary?.afternoon_minutes ?? 0,t)})}/><MetricPill icon={<Plane/>} label={t("employee.metrics.weekendHoliday")} value={shortHoursLabel(summary?.weekend_holiday_minutes ?? 0,t)} title={t("employee.metrics.weekendHolidayDetail",{hours:hoursLabel(summary?.weekend_holiday_minutes ?? 0,t)})}/>{showProcessingStatus&&<MetricPill className="employee-pill--processing" icon={isOnline?<Wifi/>:<WifiOff/>} label={t("employee.metrics.processing")} value={lockSummary||queueCount?t("employee.metrics.waiting",{count:queueCount}):t("employee.metrics.offline")} title={lockSummary||queueCount?lockSummary||t("employee.metrics.queueWaiting",{count:queueCount}):t("employee.metrics.offlineQueued")}/>}</div>
      </header>
      <div className="employee-mode-switch" role="tablist" aria-label={t("employee.page.switchView")}><button type="button" role="tab" aria-selected={view==="attendance"} className={view==="attendance"?"active":""} onClick={()=>setView("attendance")}><CalendarDays/>{t("employee.page.attendanceTab")}</button><button type="button" role="tab" aria-selected={view==="plan"} className={view==="plan"?"active":""} onClick={()=>setView("plan")}><CalendarRange/>{t("employee.page.planTab")}</button></div><PanelToolbar session={session} onSession={setSession}/>{summary&&<>{summary.plan_balance_minutes!==0&&<StatusMessage kind={summary.plan_balance_minutes<0?"offline":"success"} title={t("employee.metrics.shiftPlanBalanceTitle")}>{t(summary.plan_balance_minutes<0?"employee.metrics.shiftPlanUnder":"employee.metrics.shiftPlanOver",{hours:hoursLabel(Math.abs(summary.plan_balance_minutes),t)})}</StatusMessage>}{summary.worked_balance_mode&&workedBalance!==null&&workedBalance!==0&&<StatusMessage kind={workedBalance<0?"offline":"success"} title={t("employee.metrics.workedBalanceTitle")}>{t(workedBalance<0?"employee.metrics.workedUnder":"employee.metrics.workedOver",{hours:hoursLabel(Math.abs(workedBalance),t)})}</StatusMessage>}</>}{notice&&<StatusMessage kind={queueCount?"offline":"success"} title={queueCount?t("employee.notices.queuedTitle"):t("employee.notices.doneTitle")}>{notice}</StatusMessage>}{(mutation.error||statusMutation.error||planMutation.error)&&<StatusMessage kind="error" title={t("employee.errors.saveFailed")}>{(mutation.error||statusMutation.error||planMutation.error)?.message}</StatusMessage>}{query.isPending&&<StatusMessage kind="loading" title={t("employee.errors.loading")}/>} {query.error&&<StatusMessage kind="error" title={t("employee.errors.loadFailed")}>{query.error.message}</StatusMessage>}{query.data&&<>{attendanceLocked&&<StatusMessage kind="offline" title="Docházka je uzamčena">Zápisy docházky jsou pro tento úvazek a měsíc pouze pro čtení.</StatusMessage>}{shiftPlanLocked&&<StatusMessage kind="offline" title="Plán služeb je uzamčen">Zápisy plánu služeb jsou pro tento úvazek a měsíc pouze pro čtení.</StatusMessage>}{view==="plan"&&!(query.data.shift_plan_editable ?? false)&&!shiftPlanLocked&&<StatusMessage kind="offline" title={t("employee.errors.planReadonlyTitle")}>{t("employee.errors.planReadonlyBody")}</StatusMessage>}<section className="ledger employee-ledger" aria-label={view==="attendance"?t("employee.page.attendanceDays"):t("employee.page.shiftPlanDays")}>{query.data.days.map(day=>view==="attendance"?<DayCard key={day.date} day={day} attendanceLocked={attendanceLocked} savedField={savedCell?.date===day.date?savedCell.field:null} onSave={(d,values,field)=>mutation.mutate({day:d,values,field})} onStatus={(d,status,confirmed)=>statusMutation.mutate({day:d,status,confirmed})}/>:<PlanDayCard key={day.date} day={day} shiftPlanLocked={shiftPlanLocked} editable={query.data.shift_plan_editable ?? false} onSave={(d,values,status)=>planMutation.mutate({day:d,values,status})}/>)}</section></>}{statusConflict&&<Modal title={t("employee.errors.conflictTitle")} description={t("employee.errors.conflictBody",{date:statusConflict.day.date})} confirmLabel={t("employee.errors.conflictConfirm")} danger onClose={()=>setStatusConflict(null)} onConfirm={()=>statusMutation.mutate({...statusConflict,confirmed:true})}/>}</div></main>{view==="attendance"&&<footer className="employee-nowbar" aria-label={t("employee.quickNow.label")}><button type="button" disabled={nowDisabled} onClick={saveNow}><TimerReset/><span>{t("employee.quickNow.button")}</span></button></footer>}
  </div>;
}
