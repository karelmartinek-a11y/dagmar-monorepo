...
const dayName = new Intl.DateTimeFormat("cs-CZ", { weekday:"long", timeZone:"Europe/Prague" });
...
const namedays:Record<string,string>={
  ...
  "07-01":"Jaroslava","07-02":"Patricie","07-03":"Radomír","07-04":"Prokop","07-05":"Cyril a Metoděj","07-06":"Den upálení mistra Jana Husa","07-07":"Bohuslava","07-08":"Nora","07-09":"Drahoslava","07-10":"Libuše","07-11":"Olga","07-12":"Bořek","07-13":"Markéta","07-14":"Karolína","07-15":"Jindřich","07-16":"Luboš","07-17":"Martina","07-18":"Drahomíra","07-19":"Čeněk","07-20":"Ilja","07-21":"Vítězslav","07-22":"Magdaléna","07-23":"Libor","07-24":"Kristýna","07-25":"Jakub","07-26":"Anna","07-27":"Věroslav","07-28":"Viktor","07-29":"Marta","07-30":"Bořivoj","07-31":"Ignác",
...
export function EmployeePage(){
  ...
  const nowDisabled=!today||!nowField||query.data?.locked||!today.is_within_employment_period||today.planned_status==="HOLIDAY"||today.planned_status==="OFF"||mutation.isPending;
  const saveNow=()=>{const current=pragueNowParts();const day=query.data?.days.find(item=>item.date===current.date);if(!day||query.data?.locked||!day.is_within_employment_period||day.planned_status==="HOLIDAY"||day.planned_status==="OFF")return;const field=nextEmptyTimeField(day);if(!field)return;mutation.mutate({day,values:{...dayToValues(day),[field]:current.time},field})};
  return <div className={`employee-app ${inverted?"employee-app--light":""}`}><header className="topbar employee-topbar"><Brand compact/><span>{session.display_name}</span><div className="employee-topbar__actions"><Button variant="quiet" onClick={()=>setInverted(value=>!value)} aria-label="Inverze barev"><Contrast/><span>Inverze</span></Button><Button variant="quiet" onClick={logout} aria-label="Odhlásit"><LogOut/><span>Odhlásit</span></Button></div></header><main id="main-content" className="employee-main"><div className="page employee-page"><h1 className="sr-only">Měsíční docházka</h1><header className="employee-command" aria-label="Souhrn měsíční docházky"><button className="icon-button" aria-label="Předchozí měsíc" onClick={()=>moveMonth(-1)} title="Předchozí měsíc"><ChevronLeft/></button><div className="employee-command__month" title={`Zobrazený měsíc ${monthName.format(month)}`}><CalendarDays/><strong>{monthName.format(month)}</strong></div><MetricPill icon={<CalendarClock/>} label="Plán hodin" value={shortHoursLabel(plannedMinutes)} title={`Plán hodin: ${hoursLabel(plannedMinutes)}`}/><MetricPill icon={<BriefcaseBusiness/>} label="Odpracováno" value={shortHoursLabel(actualMinutes)} title={`Odpracováno: ${hoursLabel(actualMinutes)} za ${filledDays} vyplněných dnů`}/><MetricPill icon={<Plane/>} label="Dovolená" value={`${holidayDays} d`} title={`Dovolená v měsíci: ${holidayDays} dní`}/><MetricPill className="employee-pill--processing" icon={navigator.onLine?<Wifi/>:<WifiOff/>} label="Zpracování" value={query.data?.locked?"Zámek":queueCount?`Čeká ${queueCount}`:navigator.onLine?"Online":"Offline"} title={query.data?.locked?"Měsíc je uzamčený":queueCount?`Ve frontě čeká ${queueCount} operací`:navigator.onLine?"Online, změny se ukládají přímo":"Offline, změny se uloží do fronty"}/><button className="icon-button" aria-label="Následující měsíc" onClick={()=>moveMonth(1)} title="Následující měsíc"><ChevronRight/></button></header><div className="employee-mode-switch" role="tablist" aria-label="Přepnutí zobrazení"><button type="button" role="tab" aria-selected={view==="attendance"} className={view==="attendance"?"active":""} onClick={()=>setView("attendance")}><CalendarDays/>Docházka</button><button type="button" role="tab" aria-selected={view==="plan"} className={view==="plan"?"active":""} onClick={()=>setView("plan")}><CalendarRange/>Plán služeb</button></div><PanelToolbar session={session} onSession={setSession}/>{notice&&<StatusMessage kind={queueCount?"offline":"success"} title={queueCount?"Změna je ve frontě":"Hotovo"}>{notice}</StatusMessage>}{(mutation.error||statusMutation.error||planMutation.error)&&<StatusMessage kind="error" title="Změnu nelze uložit">{(mutation.error||statusMutation.error||planMutation.error)?.message}</StatusMessage>}{query.isPending&&<StatusMessage kind="loading" title="Načítám docházku"/>}{query.error&&<StatusMessage kind="error" title="Docházku nelze načíst">{query.error.message}</StatusMessage>}{query.data&&<>{query.data.locked&&<StatusMessage kind="offline" title="Měsíc je uzamčen">Záznamy jsou dostupné pouze pro čtení.</StatusMessage>}{view==="plan"&&!(query.data.shift_plan_editable ?? false)&&<StatusMessage kind="offline" title="Plán služeb je pouze pro čtení">Zadávání plánu služeb pro tento úvazek a měsíc musí povolit administrátor.</StatusMessage>}<section className="ledger employee-ledger" aria-label={view==="attendance"?"Dny měsíce docházky":"Dny měsíce plánu služeb"}>{query.data.days.map(day=>view==="attendance"?<DayCard key={day.date} day={day} locked={query.data.locked} savedField={savedCell?.date===day.date?savedCell.field:null} onSave={(d,values,field)=>mutation.mutate({day:d,values,field})} onStatus={(d,status,confirmed)=>statusMutation.mutate({day:d,status,confirmed})}/>:<PlanDayCard key={day.date} day={day} locked={query.data.locked} editable={query.data.shift_plan_editable ?? false} onSave={(d,values,status)=>planMutation.mutate({day:d,values,status})}/>)}</section></>}{statusConflict&&<Modal title="Změnit stav dne a odstranit kolidující data?" description={`Den ${statusConflict.day.date} obsahuje docházku nebo plán, který se zvoleným stavem koliduje.`} confirmLabel="Změnit stav a odstranit konflikty" danger onClose={()=>setStatusConflict(null)} onConfirm={()=>statusMutation.mutate({...statusConflict,confirmed:true})}/>}</div></main>{view==="attendance"&&<footer className="employee-nowbar" aria-label="Rychlý zápis aktuálního času"><button type="button" disabled={nowDisabled} onClick={saveNow}><TimerReset/><span>TEĎ</span></button></footer>}</div>;
}
