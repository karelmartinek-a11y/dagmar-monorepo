...
export function AdminLoginPage() {
  ...
  return <AuthFrame title="Vstup do administrace" description="Přihlášení je chráněno serverovou session a CSRF kontrolou."><form onSubmit={submit}><Field label="E-mail administrátora"><input type="email" autoComplete="username" required value={username} onChange={e=>setUsername(e.target.value)} /></Field><Field label="Heslo"><input type="password" autoComplete="current-password" required value={password} onChange={e=>setPassword(e.target.value)} /></Field>{error && <StatusMessage kind="error" title="Přihlášení nebylo přijato">{error}</StatusMessage>}<Button disabled={pending}>{pending ? "Ověřuji…" : "Přihlásit do administrace"}</Button></form></AuthFrame>;
}
