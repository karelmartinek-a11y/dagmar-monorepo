...
`/admin/tisky/preview?type=attendance` vykresluje pro každý vybraný `employment_id` a měsíc jednu A4 stránku na výšku podle schválené předlohy docházkového listu. Náhled i browserové uložení do PDF používají stejnou DOM kompozici: značku, identifikační mřížku, jeden řádek na kalendářní den, chronologické časové sloupce `PRŮCHOD` a souhrn pouze aktivních `display_metrics`. Nedostupné údaje předlohy se nevymýšlejí ani nedopočítávají.
