...
`/admin/tisky/preview?type=attendance` vykresluje pro každý vybraný `employment_id` a měsíc jednu A4 stránku na výšku podle schválené předlohy docházkového listu. Náhled i browserové uložení do PDF používají stejnou DOM kompozici: černý pruh se jménem zaměstnance, jediný titul `OBDOBÍ: MĚSÍC RRRR`, identifikační mřížku pouze s typem úvazku, platností úvazku a názvem úvazku, jeden řádek na kalendářní den, chronologické časové sloupce `PRŮCHOD` a pevnou masku pěti intervalových metrik v pořadí backendového kontraktu. Neaktivní metriky jsou v masce výslovně označené jako nerelevantní, nikoli skryté. Spodní souhrn obsahuje stejnou pevnou masku, relevantní statusové kredity a měsíční plán z pole `planned` dodaného backendem; chybějící backendová hodnota zůstává `—`, nikoli falešná nula. Podpisová pole se netisknou; zápatí obsahuje lokalizovaný text KájovoDagmar a čas generování. Frontend ani tisk žádnou hodnotu nedopočítává a nedostupné údaje se nevymýšlejí.
